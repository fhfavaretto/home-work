...
/*/{Protheus.doc} WSRESTFUL ZWCLIENTES
...
WSRESTFUL ZWCLIENTES DESCRIPTION 'CADASTRO DE CLIENTES'
    //Atributos
    WSDATA id         AS STRING
    ...
    //Métodos
    WSMETHOD GET    ID     DESCRIPTION 'Retorna o registro pesquisado' WSSYNTAX '/ZWCLIENTES/get_id?{id}'                       PATH 'get_id'        PRODUCES APPLICATION_JSON
    WSMETHOD GET    ALL    DESCRIPTION 'Retorna todos os registros'    WSSYNTAX '/ZWCLIENTES/get_all?{updated_at, limit, page}' PATH 'get_all'       PRODUCES APPLICATION_JSON
    WSMETHOD POST   NEW    DESCRIPTION 'Inclusão de registro'          WSSYNTAX '/ZWCLIENTES/new'                               PATH 'new'           PRODUCES APPLICATION_JSON
    WSMETHOD PUT    UPDATE DESCRIPTION 'Atualização de registro'       WSSYNTAX '/ZWCLIENTES/update'                            PATH 'update'        PRODUCES APPLICATION_JSON
    WSMETHOD DELETE ERASE  DESCRIPTION 'Exclusão de registro'          WSSYNTAX '/ZWCLIENTES/erase'                             PATH 'erase'         PRODUCES APPLICATION_JSON
END WSRESTFUL
...
WSMETHOD GET ID WSRECEIVE id WSSERVICE ZWCLIENTES
    Local lRet       := .T.
    ...
    Else
        ...
        Else
            ...
            jResponse['telex'] := (cAliasWS)->A1_TELEX 
...
WSMETHOD GET ALL WSRECEIVE updated_at, limit, page WSSERVICE ZWCLIENTES
    Local lRet       := .T.
    Local jResponse  := JsonObject():New()
    Local cQueryTab  := ''
    Local nTamanho   := 10
    ...
    Else
        ...
        While ! QRY_TAB->(EoF())
            ...
            oRegistro['telex'] := (cAliasWS)->A1_TELEX 
...
WSMETHOD POST NEW WSRECEIVE WSSERVICE ZWCLIENTES
    Local lRet              := .T.
    Local aDados            := {}
    Local jJson             := Nil
    ...
    Else
		...
		aAdd(aDados, {'A1_TELEX',   jJson:GetJsonObject('telex'),   Nil})
		...
		If lMsErroAuto
			...
			cArqLog := 'ZWCLIENTES_New_' + dToS(Date()) + '_' + StrTran(Time(), ':', '-') + '.log'
...
WSMETHOD PUT UPDATE WSRECEIVE id WSSERVICE ZWCLIENTES
    Local lRet              := .T.
    Local aDados            := {}
    Local jJson             := Nil
    Local cJson             := Self:GetContent()
...
WSMETHOD DELETE ERASE WSRECEIVE id WSSERVICE ZWCLIENTES
    Local lRet              := .T.
    Local aDados            := {}
    Local jJson             := Nil
    Local cJson             := Self:GetContent()
